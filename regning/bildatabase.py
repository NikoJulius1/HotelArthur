...
cursor_booking =conn_billing.cursor()
cursor_billing = sqlite3.cursor()


cursor_billing.execute(''' CREATE TABLE IF NOT EXISTS billing (
...
def generate_bill(booking_id):
    cursor_booking.execute('SELECT category, checkin, checkout FROM products WHERE id = ?',(booking_id))
